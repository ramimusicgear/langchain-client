...
def filter_by_basic_filters(at):
    ...
    assert (
        at.session_state.filters
        and "0.0.2" in at.session_state.filters["backend_versions"]
    )
    assert all(
        (
            chat["subcategory"] == "Electric Guitars"
            and chat["backend_version"] == "0.0.2"
    ...
    assert (
        at.session_state.filters
        and "Electric Guitars" not in at.session_state.filters["subcategories"]
    )
    assert (
        at.session_state.filters
        and "0.0.3" in at.session_state.filters["backend_versions"]
    )
    assert all(
        chat["backend_version"] == "0.0.3" for chat in at.session_state.conversations
    )
    at.sidebar.button("change_database_collection_btn").click().run()
